...
def _bind_single_video(args: Any, output_dir: Path) -> None:
    """Point --videos-dir at a one-file staging directory when --video is set.

    The staging directory is under the run output so a sibling of the source
    file (for example tests/assets with several clips) is not ingested.
    """
    ...
    staging = output_dir / ".single-video"
    staging.mkdir(parents=True, exist_ok=True)
    dest = staging / path.name
    if dest.exists() or dest.is_symlink():
        dest.unlink()
    dest.symlink_to(path)
    args.videos_dir = str(staging)
...
def apply_local_env(args: Any) -> None:
    ...
    _bind_single_video(args, output_dir)
