...
def project_roots(anchor_file: str) -> tuple[Path, Path]:
    """Return (selfsuvis package dir, marker repo root) for layered env loading.

    Depth is not hard-coded: the same helper works from the monorepo
    ``src/selfsuvis/...`` layout and from a staged
    ``packages/<name>/src/selfsuvis/...`` layout.
    """
    current = Path(anchor_file).resolve()
    package_root = None
    for parent in current.parents:
        if parent.name == "selfsuvis" and (
            (parent / "pipeline").is_dir() or (parent / "env").is_dir()
        ):
            package_root = parent
            break
    if package_root is None:
        package_root = current.parent
    repo_root = kit_project_root(current)
