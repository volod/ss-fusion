...
def generate_report(
    ...
    if "timeline" in charts:
        ...
        if ts:
            peak_ts = [
                f"{summary.frames[i].t_sec:.1f}s" for i in ts.peak_frames if i < len(summary.frames)
            ]
            sections.append(
    ...
    if "detections" in charts:
        ...
        if ds:
            by_class_rows = "".join(
                f"<tr><td>{k}</td><td>{v}</td></tr>" for k, v in ds.by_class.items()
            )
            sections.append(f"<table><tr><th>Class</th><th>Count</th></tr>{by_class_rows}</table>")
    ...
    if artifacts:
        ...
        sections.append(
            f"<h2>Output Artifacts</h2><table><tr><th>File</th><th>Size</th></tr>{rows}</table>"
        )
