...
CATALOGS: dict[str, list[ModelEntry]] = {
    ...
    # ── Object Detection ─────────────────────────────────────────────────────
    "detection": [
        ModelEntry("facebook/detr-resnet-50",             0.041, 0.1,
                   "DETR-ResNet50 — classic transformer detection, COCO 42 mAP"),
        ModelEntry("PekingU/rtdetr_r50vd",                0.042, 0.1,
                   "RT-DETR-R50 — real-time DETR, 53.1 mAP @ 108 FPS on T4"),
        ModelEntry("PekingU/rtdetr_r101vd",               0.076, 0.2,
                   "RT-DETR-R101 — larger backbone, 54.3 mAP"),
        ModelEntry("omlab/omdet-turbo-swin-tiny-hf",      0.108, 0.3,
                   "OmDet-Turbo-Tiny — open-vocabulary zero-shot detection"),
        ModelEntry("omlab/omdet-turbo-swin-large-hf",     0.218, 0.5,
                   "OmDet-Turbo-Large — open-vocabulary, best speed/accuracy"),
        ModelEntry("IDEA-Research/grounding-dino-tiny",   0.173, 0.4,
                   "Grounding DINO Tiny — open-vocab, text-guided detection"),
        ModelEntry("IDEA-Research/grounding-dino-base",   0.341, 0.7,
                   "Grounding DINO Base — open-vocab, stronger accuracy"),
        ModelEntry("microsoft/conditional-detr-resnet-101", 0.062, 0.2,
                   "Conditional DETR-R101 — faster convergence than DETR"),
        ModelEntry("jozhang97/deta-swin-large",           0.218, 0.5,
                   "DETA-Swin-Large — anchor-based DETR, 63.5 COCO AP"),
        ModelEntry("SenseTime/deformable-detr",           0.040, 0.1,
                   "Deformable-DETR — sparse attention, fast convergence"),
    ],

    # ── Image Segmentation ───────────────────────────────────────────────────
    "segmentation": [
        ModelEntry("facebook/sam2-hiera-tiny",            0.038, 0.1,
                   "SAM2-Tiny — fastest SAM2, video + image, interactive",
                   supports_video=True),
        ModelEntry("facebook/sam2-hiera-small",           0.046, 0.1,
                   "SAM2-Small — good quality, real-time capable",
                   supports_video=True),
        ModelEntry("facebook/sam-vit-base",               0.093, 0.2,
                   "SAM-Base — Segment Anything, prompt-based"),
        ModelEntry("facebook/sam-vit-large",              0.308, 0.6,
                   "SAM-Large — strong edge quality"),
        ModelEntry("CIDAS/clipseg-rd64-refined",          0.071, 0.2,
                   "CLIPSeg — text + click guided segmentation"),
        ModelEntry("nvidia/segformer-b5-finetuned-ade-512-512", 0.085, 0.2,
                   "SegFormer-B5 — semantic segmentation, ADE20k 84.0 mIoU"),
        ModelEntry("facebook/sam-vit-huge",               0.641, 1.3,
                   "SAM-Huge — best SAM1 quality"),
        ModelEntry("facebook/sam2-hiera-large",           0.224, 0.5,
                   "SAM2-Large — best SAM2 quality, video tracking",
                   supports_video=True),
        ModelEntry("shi-labs/oneformer_coco_swin_large",  0.219, 0.5,
                   "OneFormer — panoptic + semantic + instance, single model"),
        ModelEntry("openmmlab/mask2former-swin-large-coco-panoptic", 0.216, 0.5,
    ...
    "vqa": [
        ...
        ModelEntry("allenai/MolmoE-1B-0924",              1.0,   2.0,
                   "MolmoE-1B — mixture-of-experts VLM, strong pointing"),
        ModelEntry("Qwen/Qwen2.5-VL-3B-Instruct",        3.0,   6.0,
                   "Qwen2.5-VL-3B — compact, strong grounding+OCR"),
        ModelEntry("llava-hf/llava-1.5-7b-hf",           7.0,  14.0,
                   "LLaVA-1.5-7B — instruction-following VLM"),
        ModelEntry("allenai/Molmo-7B-D-0924",             7.0,  14.0,
                   "Molmo-7B-D — strong spatial reasoning + pointing"),
        ModelEntry("Qwen/Qwen2.5-VL-7B-Instruct",        7.0,  14.0,
                   "Qwen2.5-VL-7B — already in pipeline as Phase 2 engine"),
        ModelEntry("microsoft/Phi-3.5-vision-instruct",  4.2,   8.5,
                   "Phi-3.5-Vision — 128K context, strong reasoning"),
        ModelEntry("llava-hf/llava-1.5-13b-hf",         13.0,  26.0,
                   "LLaVA-1.5-13B — best open LLaVA quality"),
        ModelEntry("Qwen/Qwen2.5-VL-32B-Instruct",      32.0,  64.0,
                   "Qwen2.5-VL-32B — near GPT-4V quality, needs A100"),
        ModelEntry("Qwen/Qwen2.5-VL-72B-Instruct",      72.0, 144.0,
                   "Qwen2.5-VL-72B — top open-source VLM, needs 2×A100"),
    ],

    # ── Zero-shot Classification (CLIP / SigLIP) ─────────────────────────────
    "zero_shot_classification": [
        ModelEntry("openai/clip-vit-base-patch32",             0.151, 0.3,
                   "CLIP ViT-B/32 — fastest OpenAI CLIP, already in pipeline"),
        ModelEntry("openai/clip-vit-base-patch16",             0.151, 0.3,
                   "CLIP ViT-B/16 — finer patches, better spatial understanding"),
        ModelEntry("google/siglip-base-patch16-224",           0.200, 0.4,
                   "SigLIP-Base — sigmoid loss, better than CLIP on zero-shot"),
        ModelEntry("laion/CLIP-ViT-B-32-laion2B-s34B-b79K",   0.151, 0.3,
                   "CLIP ViT-B/32 on LAION-2B — improved open-world coverage"),
        ModelEntry("openai/clip-vit-large-patch14",            0.428, 0.9,
                   "CLIP ViT-L/14 — strong baseline, 23M+ downloads"),
        ModelEntry("openai/clip-vit-large-patch14-336",        0.428, 0.9,
                   "CLIP ViT-L/14 @336px — higher resolution input"),
        ModelEntry("laion/CLIP-ViT-H-14-laion2B-s32B-b79K",   0.986, 2.0,
                   "CLIP ViT-H/14 on LAION-2B — largest standard CLIP"),
        ModelEntry("google/siglip-so400m-patch14-384",         0.878, 1.8,
                   "SigLIP-SO400M — 400M patch encoder, top zero-shot accuracy"),
        ModelEntry("google/siglip2-so400m-patch14-384",        0.878, 1.8,
                   "SigLIP2-SO400M — improved version with better calibration"),
        ModelEntry("laion/CLIP-ViT-g-14-laion2B-s34B-b88K",   1.366, 2.7,
    ...
    "world_model": [
        ModelEntry("google/vivit-b-16x2-kinetics400",            0.086, 0.2,
                   "ViViT-B — pure-transformer video classification, Kinetics-400",
                   supports_video=True),
        ModelEntry("facebook/timesformer-base-finetuned-k400",   0.122, 0.3,
                   "TimeSformer-Base — divided space-time attention, Kinetics-400",
                   supports_video=True),
        ModelEntry("MCG-NJU/videomae-base",                      0.122, 0.3,
                   "VideoMAE-Base — masked autoencoder pretraining on video",
                   supports_video=True),
        ModelEntry("google/videoprism-base-f16r288",             0.300, 0.7,
                   "VideoPrism-Base — Google dual-encoder video-text model",
                   supports_video=True),
        ModelEntry("MCG-NJU/videomae-large",                     0.307, 0.6,
                   "VideoMAE-Large — larger backbone, stronger video features",
                   supports_video=True),
        ModelEntry("facebook/vjepa2-vitl-fpc64-256",             0.307, 0.7,
                   "V-JEPA2-ViT-L — Facebook self-supervised video JEPA, 64 frames",
                   supports_video=True),
        ModelEntry("OpenGVLab/VideoMAEv2-Huge",                  0.600, 1.3,
                   "VideoMAE-v2-Huge — stronger v2 pretraining, 600M params",
                   supports_video=True),
        ModelEntry("facebook/vjepa2-vitg-fpc64-256",             1.0,   2.0,
