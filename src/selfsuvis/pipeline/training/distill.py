...
class DistillConfig:
    ...
    caption_embeddings: Optional[np.ndarray] = None  # (N_frames, clip_dim)
...
class KnowledgeDistiller:
    ...
    def distill(
        ...
        loss_components: Dict[str, List[float]] = {
            "rkd_d": [], "rkd_a": [], "cosine": [], "koleo": [], "caption": [],
        }

        # Caption anchor tensors — one per frame, aligned with dataset order
        _cap_anchors: Optional[torch.Tensor] = None
        if (cfg.lambda_caption_anchor > 0
            ...
            if len(_cap_np) == len(dataset):
                ...
                logger.info(
                    ...
                    cfg.lambda_caption_anchor, len(_cap_np), _cap_np.shape[1],
                )
            else:
                logger.warning(
                    "Caption anchor count (%d) != frame count (%d) — disabling anchor loss",
                    len(_cap_np), len(dataset),
                )
        t0 = time.time()
        ...
        for epoch in range(1, cfg.epochs + 1):
                ...
                if _cap_anchors is not None and cfg.lambda_caption_anchor > 0:
                    ...
                    if end > start:
                        ...
                        s_cap_proj = F.normalize(
                            self._cap_proj(s_for_cap.float()), dim=-1
                        )
            ...
            cap_str = (f"  cap={loss_components['caption'][-1]:.4f}"
                       if _cap_anchors is not None else "")
            logger.info(
