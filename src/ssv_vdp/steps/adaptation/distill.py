...
def step_distill(
    ...
    # DINOv3 teacher and DINOv2 student backbones default to xFormers
    # memory-efficient attention, whose kernels reject float32 on newer GPUs
    # (sm_90+/Blackwell: "requires device with capability <= (9, 0)" and
    # "dtype=torch.float32 (supported: bfloat16, float16)"). Distillation runs
    # both backbones in float32, so force the PyTorch SDPA path for its duration
    # — negligible cost at this scale (a few epochs over a single clip) and the
    # only path that works across GPU generations. The DinoEmbedder probe does
    # not cover these backbones because they are loaded directly via hub_load_dino.
    _restore_xformers = False
    try:
        from selfsuvis.models.dino_model import _set_dino_xformers_enabled

        _set_dino_xformers_enabled(False)
        _restore_xformers = device.startswith("cuda")
    except Exception:
        pass
    try:
